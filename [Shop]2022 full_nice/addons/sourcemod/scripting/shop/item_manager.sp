...
bool:ItemManager_FillCategories(Handle:menu, source_client, bool:inventory = false)
{
	...
	decl Handle:trie, Handle:array, any:tmp[CAT_MAX], String:sCatId[16];
	
	new bool:result = false;
	new size;
	
	for (new i = 0; i < GetArraySize(h_arCategories); i++)
	{
		if (inventory)
		{
			size = GetClientCategorySize(source_client, i);
			if (size < 1)
			{
				continue;
			}
		...
		GetArrayString(h_arCategories, i, category, sizeof(category));
		...
		ItemManager_OnCategoryDisplay(on_display[0], on_display[1], source_client, i, category, buffer, display, sizeof(display));
		
		description[0] = '\0';
		GetTrieString(trie, "description", buffer, sizeof(buffer));
		ItemManager_OnCategoryDescription(on_desc[0], on_desc[1], source_client, i, category, buffer, description, sizeof(description));
		...
		IntToString(i, sCatId, sizeof(sCatId));
